...
public class VDFNode extends LinkedHashMap<String, Object[]> {
    ...
    public String toVDFConfig() {
        return childToString(this,0).trim();
    }

    private String childToString(Object children,int format) {
        if (children instanceof VDFNode) {
            StringBuilder builder = new StringBuilder();
            for (Map.Entry<String, Object[]> entry : ((VDFNode) children).entrySet()) {
                Object value = entry.getValue()[0];
                if (value instanceof String) {
                    builder.append("\t".repeat(format)).append("\"").append(entry.getKey()).append("\"\t\t\"").append(value).append("\"").append("\r\n");
                } else {
//                    if (((VDFNode) children).containsKey("Commands")) {
//                        VDFNode commands = ((VDFNode) children).getSubNode("Commands");
//                        Map<String,Object[]> node = new LinkedHashMap<>();
//                        AtomicInteger count = new AtomicInteger(1);
//                        while (node.size() < commands.size()) {
//                            for (Map.Entry<String, Object[]> e : commands.entrySet()) {
//                                Object v = e.getValue()[0];
//                                if (v instanceof VDFNode) {
//                                    String hotkey = ((VDFNode) v).getString("hotkey");
//                                    int i = Integer.parseInt(hotkey);
//                                    if (count.get() == i && !node.containsKey(e.getKey())) {
//                                        count.incrementAndGet();
//                                        System.out.println("PutKey: " + e.getKey());
...
//                        }
//                        commands.clear();
//                        commands.putAll(node);
//                    }
                    builder.append("\t".repeat(format)).append("\"").append(entry.getKey()).append("\"")
                            .append("\r\n").append("\t".repeat(format)).append("{")
                            .append("\r\n").append(childToString(value,format + 1))
                            .append("\t".repeat(format)).append("}").append("\r\n");
                }
            }
