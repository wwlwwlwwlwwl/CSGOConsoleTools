...
public class JavaSoundAudioDevice extends AudioDeviceBase
{
	...
	private static final List<MusicEventListener> EVENT_LIST = new ArrayList<>();

	public static void listenEvent(MusicEventListener r) {
		if (r == null) {
			return;
		}
		EVENT_LIST.add(r);
	...
	public static void fireEvent(MusicEvent event) {
		if (event == null) {
			return;
		}
	...
	protected void createSource() throws JavaLayerException
    {
        ...
        try
        {
            ...
            if (line instanceof SourceDataLine)
            {
                ...
                source.start();
				MusicEvent event = new MusicEvent(-10.0F, MusicEvent.EventType.SETTING_VOLUME);
				fireEvent(event);
				if (!event.isCanceled()) {
					setLineGain(event.getVolume());
				}
            }
